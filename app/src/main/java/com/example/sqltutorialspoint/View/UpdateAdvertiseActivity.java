...
public class UpdateAdvertiseActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    StepView sv;
    ...
    String type_hostel,seat_no,flat_space,room_no,roommate_no,roommate_type,desc;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new android.os.Handler().postDelayed(
                new Runnable() {
                    public void run() {
                        try {
                            title=advertisePresenter.getTitle(ad_id);
                            ad_type=advertisePresenter.getType(ad_id);
                            location=advertisePresenter.getLocation(ad_id);
                            price=advertisePresenter.getPrice(ad_id);
                            address=advertisePresenter.getAddress(ad_id);
                            contact_no=advertisePresenter.getContact(ad_id);
                            bm[0]=advertisePresenter.getImage(ad_id);
                            ...
                            roommate_no=advertisePresenter.getRoommateSeatNo(ad_id);
                            roommate_type=advertisePresenter.getRoommateType(ad_id);
                            desc=advertisePresenter.getDescription(ad_id);

                            input_heading.setText(title);
                            input_price.setText(price);
                            input_address.setText(address);
                            input_contact_no.setText(contact_no);
                            input_location.setText(location);
                            setSpinText(spinner,ad_type);
                            img1.setImageBitmap(bm[0]);
                            if(ad_type.equalsIgnoreCase("hostel")){
                                input_no_seat.setText(seat_no);
                                if(type_hostel.equalsIgnoreCase("male"))
                                    male.setChecked(true);
                                else
                                    female.setChecked(true);
                                input_desc1.setText(desc);
                            }
                            else if(ad_type.equalsIgnoreCase("flat")){
                                input_flat_room_no.setText(room_no);
                                input_flat_size.setText(flat_space);
                                input_desc2.setText(desc);
                            }else if(ad_type.equalsIgnoreCase("roommate")){
                                input_no_seat.setText(roommate_no);
                                if(roommate_type.equalsIgnoreCase("male"))
                                    male.setChecked(true);
                                else
                                    female.setChecked(true);
                                input_desc1.setText(desc);
    ...
    private void selectImage() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(UpdateAdvertiseActivity.this);
        builder.setTitle("Add Photo!");
        builder.setItems(items, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int item) {
                boolean result= util_functions.checkPermission(UpdateAdvertiseActivity.this);
    ...
    public void goThirdStep(View view) throws ExecutionException, InterruptedException {
        ...
        if (bm[0] == null) {
            ...
            if (ad_type.equalsIgnoreCase("hostel")) {
                ...
                if(features.charAt(1)=='1')
                    checkBalcony.setChecked(true);
                else
                    checkBalcony.setChecked(false);
                if(features.charAt(2)=='1')
                    checkDining.setChecked(true);
                else
                    checkDining.setChecked(false);
                if(features.charAt(3)=='1')
                    checkSweeper.setChecked(true);
                else
                    checkSweeper.setChecked(false);
                ...
                if(features.charAt(1)=='1')
                    ...
                    checkBalcony.setChecked(false);
                if(features.charAt(2)=='1')
                    checkGarage.setChecked(true);
                else
                    checkGarage.setChecked(false);
                if(features.charAt(3)=='1')
                    checkExit.setChecked(true);
                else
                    checkExit.setChecked(false);
    ...
    public void publishAd(View view) {
        ...
        if(ad_type.equalsIgnoreCase("flat")){
            ...
            if(checkExit.isChecked())
                ...
                features=features+"0";        }
        new androidx.appcompat.app.AlertDialog.Builder(UpdateAdvertiseActivity.this)
                .setMessage("Are you sure to Modify this Ad?")
                ...
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        final ProgressDialog progressBar= new ProgressDialog(UpdateAdvertiseActivity.this);
                        progressBar.setCancelable(true);
                        progressBar.setMessage("Please wait...");
                        progressBar.setProgressStyle(ProgressDialog.STYLE_SPINNER);
                        progressBar.setProgress(0);
...
//                                        }
                                    ...
                                    if (bm[0] != null) {
                                        ...
                                        if(advertisePresenter.deleteAd(ad_id)&& advertisePresenter.insertAdvertise(bm[0], curr_time + "_1.png", title, member_id, location, ad_type, price, address,contact_no,desc)){

                                            String ad_Id=advertisePresenter.getMaxId(ad_type.toLowerCase());
                                            advertisePresenter.updateFeatures(ad_Id,features);
                                            advertisePresenter.updateModified(ad_Id,"1");
                                            if(ad_type.equalsIgnoreCase("hostel"))
                                            {
                                            ...
                                            Toast.makeText(UpdateAdvertiseActivity.this, "Successfully Modified", Toast.LENGTH_LONG).show();
                                            Intent intent = new Intent(UpdateAdvertiseActivity.this, FirstActivity.class);
                                            intent.putExtra("id", member_id);
                                            startActivity(intent);
                                            finish();
                                        } else {
                                            Toast.makeText(UpdateAdvertiseActivity.this, "Something Went Wrong!", Toast.LENGTH_LONG).show();
                                        }
                                    }
